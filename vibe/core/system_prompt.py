...
def _get_le_chaton_section() -> str:
    return (
        ...
        "explore (grep/read), research (+web), reviewer (+bash), debugger (+bash; "
        "root-cause analysis), planner (grep/read; phased plan), security (+bash; "
        ...
        "Synthesis helpers are also injected: `flatten(items)`, "
        "`dedup_by(items, key)` (drop duplicates keeping first), and "
        "`merge_by(items, key, merge)` (group-and-fold). Use them instead of "
        "reinventing flatten/dedup/merge comprehensions in every script.\n\n"
        ...
        "The launch result carries only run_id — the script's return_value is "
        "auto-delivered on completion AND re-readable any time via "
        "`workflow_results(run_id)`, which also exposes per-agent schema_errors. "
