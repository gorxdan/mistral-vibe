...
def _get_le_chaton_section() -> str:
    return (
        ...
        "rules (safelisted imports, no `asyncio`, `str.format()` forbidden), "
        "launch semantics (pass source inline), and result retrieval "
        "(`workflow_results(run_id)`, per-agent `schema_errors`). Read it; do "
        ...
        "normally.\n\n"
        "**Monitor:** `/workflows` — x (stop), p (pause/resume), s (save script "
        "as `/<name>` command), o (view script), Enter (drill into run/agent). "
        "In-flight agents show live token totals."
    )
