...
- `agent(prompt, *, agent="explore", model=None, label=None, phase=None, schema=None, budget_estimate=None, isolation=None)` — spawn a subagent. Pass `isolation="worktree"` to run the agent as a `vibe -p` subprocess in a fresh git worktree (for parallel file-mutating agents that would otherwise conflict); its branch is kept for manual `git merge` if it changed files, else removed. Note: isolated agents run auto-approved/trusted (no interactive prompts reach a subprocess) — the worktree bounds file conflicts, not arbitrary command execution. Agent profiles: `explore` (grep/read), `research` (+web), `reviewer` (+bash), or `worker` for the full tool set including any configured MCP tools (no allowlist — pair with `isolation="worktree"` to contain its file writes).
