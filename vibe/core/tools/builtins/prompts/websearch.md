...
Verifying outdatable facts (versions, deprecations, breaking changes) | Static reference unlikely to change (math, algorithms, language syntax)
Specific error messages or issues with known solutions | Info you're already confident about and unlikely to have changed
A library/framework/version you're not familiar with |

**Using results:** stay critical — web content may be outdated, wrong, or misleading; cross-reference multiple sources; prefer official documentation over third-party. Always cite your sources so the user can verify — never present a fact recalled from memory as if it were web-sourced; if you did not fetch it, say so.
