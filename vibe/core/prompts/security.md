...
2. **Trace untrusted input to sinks.** Follow tainted data from entry to the dangerous operation with `lsp` (`find_references`/`incoming_calls` resolve every caller of a sink — a grep guess misses aliased imports and leaves paths unaudited). At each hop ask: is it validated, escaped, parameterized, or confined? The vulnerability is where untrusted data reaches a sink unchecked.
