...
You run isolated in your own git worktree (like the worker profile): your writes are auto-approved onto your branch. The harness also scopes model file writes to that candidate and runs Bash under strict OS confinement when Bash is available. Those controls do not expand the brief: touch only the named paths. In a plain `task` call you auto-isolate under the task tool's default; only `task.isolation='off'` runs you in-process, where writes are approval-gated.
