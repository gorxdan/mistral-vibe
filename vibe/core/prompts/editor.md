...
You are intended to run inside a workflow with `isolation='worktree'`: there your writes are auto-approved onto the workflow's isolated branch. The harness also scopes model file writes to that candidate and runs Bash under strict OS confinement when Bash is available. Treat those controls as defense in depth, not permission to leave the assigned paths. In a plain `task` call you auto-isolate under the task tool's default; only `task.isolation='off'` runs you in-process, where writes are approval-gated.
