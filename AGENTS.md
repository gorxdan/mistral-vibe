...
Host-agent completeness layer (on by default; `verification_subsystem = false` to disable). `verifier` subagent (`agents/models.py`) is a verdict gate distinct from `reviewer`: proves a completed implementation works by trying to break it, emits a strict `VERDICT: PASS|FAIL|PARTIAL` with command evidence. `_get_verification_contract_section` (`system_prompt.py`) tells the host to spawn it before reporting non-trivial work done; the todo tool (`tools/builtins/todo.py`) appends a structural nudge when a 3+ item list closes without a verify step. With `trusted_verification_recipe` configured, the recipe is frozen at AgentLoop creation and no-argument `verify_work` turns a current verifier PASS plus the prebound local checks into a durable receipt; `land_work` requires that current receipt. Without a recipe, only a current state-recorded verifier PASS can authorize non-trivial `land_work`; model-authored workflow contracts gate delivery but are not landing authority, and pasted report prose cannot authorize. A `trivial: <reason>` waiver remains limited to locally confirmed documentation-only diffs. `land_work` returns the merge commit SHA but does not persist a separate landing record. The verifier is read-only (read/grep/lsp + jailed bash, reuses `_review_bash_overrides`).
